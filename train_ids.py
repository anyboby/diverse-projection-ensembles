...
if __name__ == '__main__':
    args, wandb_log_config = argp_ids.read_args()
    ...
    wandb.init(project='ids', save_code=True, dir=str(wandb_dir), config=dict(**wandb_log_config, log_version=100),
    ...
    for game_frame in t:
        ...
        if game_frame % args.sync_dqn_target_every == 0 and ids.buffer.burnedin:
            ids.sync_Q_target()
        ...
        for state, action, reward, done, j in zip(states, actions, rewards, dones, range(args.parallel_envs)):
            ...
            ids.buffer.put(state, action, reward, done, j=j)
        states = next_states
        ...
        for info, j in zip(infos, range(args.parallel_envs)):
            if 'episode_metrics' in info.keys():
                ...
                log = {'x/game_frame': game_frame + j, 'x/episode': episode_count,
                       ...
                       'running_avg_return': np.mean(returns), 'lr': ids.opt.param_groups[0]['lr'], 'reward_density': reward_density,
        ...
        if game_frame % (2_499_000-(2_499_000 % args.parallel_envs)) == 0 and game_frame > 0:
            ids.save(game_frame, args=args, run_name=wandb.run.name, run_id=wandb.run.id, target_metric=np.mean(returns), returns_all=returns_all, q_values_all=q_values_all)
