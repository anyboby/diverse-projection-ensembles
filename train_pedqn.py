...
# os.environ["CUDA_LAUNCH_BLOCKING"] = "1" # activate when debugging

if __name__ == '__main__':
    args, wandb_log_config = argp_pedqn.read_args()
    ...
    wandb.init(project='pedqn', save_code=True, dir=str(wandb_dir), config=dict(**wandb_log_config, log_version=100),
    ...
    failure = True
    while failure:
        try:    # @anyboby this is terrible, fix when cluster is well-behaved again
            # connect
            env = create_env(args, decorr_steps=decorr_steps)
            states = env.reset()
            failure = False
        except Exception as e:
            print("Warning: exception in env creation")
            pass
    print("success")

    pedqn = PEDQN(env, args)
    wandb.watch(pedqn.q_policy)
    ...
    verbose_stats = dict(
        q_values_c = deque(maxlen=10),
        q_values_q = deque(maxlen=10),
        q_max_c = deque(maxlen=10),
        q_max_q = deque(maxlen=10),
        z_var_c = deque(maxlen=10),
        z_var_q = deque(maxlen=10),
        q_loss_c = deque(maxlen=10),
        q_loss_q = deque(maxlen=10),
        u_values_c = deque(maxlen=10),
        u_values_q = deque(maxlen=10),
        u_max_c = deque(maxlen=10),
        u_max_q = deque(maxlen=10),
        u_min_c = deque(maxlen=10),
        u_min_q = deque(maxlen=10),
        zu_var_c = deque(maxlen=10),
        zu_var_q = deque(maxlen=10),
        u_loss_c = deque(maxlen=10),
        u_loss_q = deque(maxlen=10),
        reward_i = deque(maxlen=10),
        reward_i_max = deque(maxlen=5),
        reward_i_min = deque(maxlen=5),
        optimistic_overturn = deque(maxlen=10),
        optimistic_churn = deque(maxlen=10),
        online_churn = deque(maxlen=10),
    )
    ...
    for game_frame in t:
        ...
        if game_frame % args.sync_dqn_target_every == 0 and pedqn.buffer.burnedin:
            pedqn.sync_Q_target()
        ...
        for state, action, reward, done, j in zip(states, actions, rewards, dones, range(args.parallel_envs)):
            ...
            pedqn.buffer.put(state, action, reward, done, j=j)
        states = next_states
        ...
        for info, j in zip(infos, range(args.parallel_envs)):
            if 'episode_metrics' in info.keys():
                ...
                log = {'x/game_frame': game_frame + j, 'x/episode': episode_count,
                       ...
                       'running_avg_return': np.mean(returns), 'lr': pedqn.opt.param_groups[0]['lr'], 'reward_density': reward_density,
                       'discounted_return': np.mean(discounted_returns)}

                if args.verbosity>1:
                    for k,v in verbose_stats.items():
                        log[k] = np.mean(v)
        ...
        if game_frame % (19_000-(19_000 % args.parallel_envs)) == 0 and game_frame > 0:
            pedqn.save(game_frame, args=args, run_name=wandb.run.name, run_id=wandb.run.id, target_metric=np.mean(returns), returns_all=returns_all, q_values_all=q_values_all)
