...
class BDQN:
    ...
    def __init__(self, env, args: SimpleNamespace) -> None:
        ...
        self.q_policy = net(depth, env.action_space.n, linear_layer, ensemble_size=args.ensemble_size, prior_beta=args.prior_beta).cuda()
        self.q_target = net(depth, env.action_space.n, linear_layer, ensemble_size=args.ensemble_size, prior_beta=args.prior_beta).cuda()
        ...
        self.active_model = np.random.randint(0, args.ensemble_size, size=(args.parallel_envs,))
    ...
    def act(self, states, eps: float):
        ...
        with torch.no_grad():
            with autocast(enabled=self.use_amp):
                ...
                action_values = self.q_policy(states)
                action_values = action_values[np.arange(len(action_values)),self.active_model] # select model
    ...
    def reset_active_model(self, env_index):
        "resets active model index for an environment"
        self.active_model[env_index] = np.random.randint(0, self.ensemble_size)

    @torch.no_grad()
    def td_target(self, reward: float, next_state, done: bool):
        if self.double_dqn:
            best_action = torch.argmax(self.q_policy(next_state), dim=2)
            next_Q = torch.gather(self.q_target(next_state), dim=2, index=best_action.unsqueeze(-1)).squeeze()
            return reward.unsqueeze(-1) + self.n_step_gamma * next_Q * (1 - done.unsqueeze(-1))
        else:
            max_q = torch.max(self.q_target(next_state), dim=2)[0]
    ...
    def train(self, batch_size, beta=None) -> Tuple[float, float, float]:
        ...
        self.opt.zero_grad()
        with autocast(enabled=self.use_amp):
            td_est = torch.gather(self.q_policy(state), dim=2, index=action.view(-1,1,1).repeat(1,self.ensemble_size,1)).squeeze()
            ...
            if self.prioritized_er:
                ...
                loss = torch.mean(weights * losses.mean(-1))
            else:
