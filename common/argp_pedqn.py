...
def read_args():
    ...
    parser.add_argument('--record', type=parse_bool, default=False, help='whether to record')    
    ...
    parser.add_argument('--wandb_dir', type=str, default="wandb", help='logdir for wandb')
    parser.add_argument('--save_dir', type=str, default="checkpoints", help='logdir for checkpoints')
    ...
    parser.add_argument('--env_name', type=str, default='gym:VizdoomMyWayHomeSparse-v0',
    ...
    parser.add_argument('--verbosity', type=int, default=2, help='verbosity level, 1 is default, 2 is extended diagnostics')

    # env preprocessing settings
    parser.add_argument('--frame_skip', type=int, default=1, help='use only every nth env frame (default is to use environment specific presets)')
    parser.add_argument('--frame_stack', type=int, default=6, help='stack n frames (default is to use environment specific presets)')
    parser.add_argument('--stack_skip', type=int, default=1, help='stack n frames (default is to use environment specific presets)')
    ...
    parser.add_argument('--resolution', type=int, default=42, help='environment resolution (default is to use environment specific presets)')
    ...
    parser.add_argument('--burnin_uncertainty', type=int, default=200_000, help='how many transitions should be in the buffer before start of training of uncertainty head')
    parser.add_argument('--gamma', type=float, default=0.997, help='reward discount factor')
    parser.add_argument('--gamma_i', type=float, default=0.997, help='intrinsic reward discount factor')
    parser.add_argument('--sync_dqn_target_every', type=int, default=8000, help='sync Q target net every n frames')

    parser.add_argument('--batch_size', type=int, default=512, help='sample size when sampling from the replay buffer')
    ...
    # rainbow settings
    parser.add_argument('--network_arch', type=str, default='impala_pedqn:2',
                        help='which model architecture to use for the q-network; one of "nature", "dueling", "impala_small", "impala_pedqn:c" (c is the number of channels in impala large)')
    parser.add_argument('--n_heads', type=int, default=101, help='number of atoms for distribution support')
    parser.add_argument('--q_min', type=float, default=-2.0, help='minimal q values for categorical support')
    parser.add_argument('--q_max', type=float, default=2.0, help='maximal q values for categorical support')
    parser.add_argument('--init_beta', type=float, default=5.0, help='beta for ucb scale')
    parser.add_argument('--final_beta', type=float, default=0.01, help='beta for ucb scale')
    parser.add_argument('--beta_decay_frames', type=int, default=3_300_000, help='exploration beta decay frames, 250_000 for rainbow paper, 1M for dopamine')
    parser.add_argument('--prior_beta', type=float, default=3.0, help='prior beta for randomized prior functions')
    parser.add_argument('--normalize_ri', type=parse_bool, default=True, help='whether to normalize intrinsic rewards with a running std')
    parser.add_argument('--mean_adjustment_ri', type=float, default=1.0, help='normalize by a fraction of the mean (1=zeromean, 0=no mean normalization)')
    parser.add_argument('--episodic_ri', type=parse_bool, default=False, help='whether to treat intrinsic reward stream as episodic (U-head is not bootstrapped at episode end).')
    parser.add_argument('--ri_norm', type=int, default=1, help='Which Wasserstein norm to use for intrinsic reward.')
    ...
    parser.add_argument('--n_step', type=int, default=10, help='the n in n-step bootstrapping')
    parser.add_argument('--init_eps', type=float, default=1.0, help='initial dqn exploration epsilon')
    parser.add_argument('--final_eps', type=float, default=0.01, help='final dqn exploration epsilon')
    parser.add_argument('--eps_decay_frames', type=int, default=500_000, help='exploration epsilon decay frames, 250_000 for rainbow paper, 1M for dopamine')

    # optimizer settings
    parser.add_argument('--lr', type=float, default=0.000625, help='learning rate for adam (0.0000625 for rainbow paper/dopamine, 0.00025 for DQN/procgen paper)')
    ...
    if args.env_name.startswith('gym:Vizdoom'):
        if args.frame_skip is None: args.frame_skip = 1
        if args.frame_stack is None: args.frame_stack = 6
        if args.stack_skip is None: args.stack_skip = 4
        if args.resolution is None: args.resolution = (42, 42)
    ...
    if args.env_name.startswith('gym:'):
        ...
        if args.stack_skip is None: args.stack_skip = 1
