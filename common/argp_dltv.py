...
def read_args():
    ...
    # rainbow settings
    parser.add_argument('--network_arch', type=str, default='impala_dltv:2',
                        help='which model architecture to use for the q-network; one of "impala_dltv:c" (c is the number of channels in impala large)')
    ...
    parser.add_argument('--eps_decay_frames', type=int, default=100_000, help='exploration epsilon decay frames, 250_000 for rainbow paper, 1M for dopamine (when not using noisy-nets)')
    parser.add_argument('--init_beta', type=float, default=0.5, help='beta for ucb scale')
    parser.add_argument('--beta_decay_scale', type=float, default=0.001, help='prior beta for randomized prior functions')

    # optimizer settings
    parser.add_argument('--lr', type=float, default=0.00075, help='learning rate for adam (0.0000625 for rainbow paper/dopamine, 0.00025 for DQN/procgen paper)')
