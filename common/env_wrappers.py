...
from common.vec_envs import SubprocVecEnvNoFlatten, DummyVecEnvNoFlatten, LazyVecFrameStack, VecEnvWrapper

cv2.ocl.setUseOpenCL(False)
cv2.setNumThreads(0) #trying to prevent clash between cv2 and multiprocessing
...
class SkipVecFrameStack(VecEnvWrapper):
    """
    ...
    """
    def __init__(self, venv, num_stack, num_skip, parallel_envs, clone_arrays, lz4_compress=False):
        super().__init__(venv)
        ...
        self.frames = [deque(maxlen=num_stack*num_skip) for _ in range(parallel_envs)]
    ...
    def _get_observation(self):
        ...
        for i in range(len(self.frames)):
            assert len(self.frames[i]) == self.num_stack*self.num_skip, (len(self.frames[i]), self.num_stack)
            result.append(LazyFrames(list(self.frames[i])[0::self.num_skip], self.lz4_compress))
        return result
    ...
    def reset(self, **kwargs):
        ...
        for i, observation in enumerate(observations):
            for _ in range(self.num_stack*self.num_skip):
                self.frames[i].append(observation)
...
def create_doom_env(config, instance_seed, instance, decorr_steps):
    """ Creates a doom environment and wraps it for DeepMind-Atari style"""
    import vizdoomgym
    # env wrapping according to
    # Adversarially Guided Actor-Critic (AGAC)
    # https://github.com/yfletberliac/adversarially-guided-actor-critic
    
    env = gym.make(config.env_name[4:])
    env = TimeLimit(env, config.time_limit)

    if instance == 0 and config.record:
...
def create_env_instance(args, instance, decorr_steps):
    ...
    elif args.env_name.startswith('gym:Vizdoom'): env = create_doom_env(args, instance_seed, instance, decorr_steps)
...
def create_env(args, decorr_steps=None):
    ...
    vec_env = partial(SubprocVecEnvNoFlatten, start_method="spawn") if args.subproc_vecenv else DummyVecEnvNoFlatten
    env = vec_env(env_fns)
    env = SkipVecFrameStack(env, args.frame_stack, args.stack_skip, args.parallel_envs, clone_arrays=not args.subproc_vecenv, lz4_compress=False)    
